...
import { Box, Typography, Checkbox, LinearProgress } from '@mui/material';
...
export default function BountiesWidget() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ mb: 4 }}>
        <LinearProgress variant="determinate" value={progress} sx={{ height: 6, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.1)', '& .MuiLinearProgress-bar': { backgroundColor: '#22D3EE', borderRadius: 3, boxShadow: '0 0 10px rgba(34,211,238,0.5)' } }} />
      </Box>

      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto', pr: 1, '&::-webkit-scrollbar': { width: '4px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '4px' } }}>
        <AnimatePresence>
          {quests.map(quest => (
            <motion.div key={quest.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }}>
              <Box sx={{ 
                ...
                bgcolor: quest.completed ? 'rgba(34,211,238,0.05)' : 'rgba(0,0,0,0.2)',
                border: '1px solid', borderColor: quest.completed ? 'rgba(34,211,238,0.2)' : 'rgba(255,255,255,0.05)',
                transition: 'all 0.2s ease', '&:hover': { borderColor: 'rgba(34,211,238,0.4)', bgcolor: 'rgba(34,211,238,0.02)' }
              }}>
                <Checkbox 
                  checked={quest.completed} 
                  onChange={() => toggleQuest(quest.id)} 
                  sx={{ color: 'rgba(255,255,255,0.3)', '&.Mui-checked': { color: '#22D3EE' } }} 
                />
                <Box sx={{ flex: 1 }}>
                  <Typography variant="body2" fontWeight={800} color={quest.completed ? "rgba(255,255,255,0.5)" : "white"} sx={{ textDecoration: quest.completed ? 'line-through' : 'none' }}>
                    {quest.text}
                  </Typography>
