...
export default function FocusTimerWidget() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center' }}>
      ...
      {/* Tabs */}
      <Box sx={{ display: 'flex', gap: 1, bgcolor: 'rgba(0,0,0,0.3)', p: 0.5, borderRadius: '100px', mb: 4 }}>
        {[
          ...
          <Button 
            ...
            sx={{ 
              ...
              color: mode === m.id ? '#8B5CF6' : 'rgba(255,255,255,0.4)',
              '&:hover': { bgcolor: mode === m.id ? 'rgba(139,92,246,0.3)' : 'rgba(255,255,255,0.1)' }
            }}
          >
            {m.label}
          </Button>
      ...
      <Box sx={{ position: 'relative', width: 220, height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 4 }}>
        ...
        <Box sx={{ 
          width: '100%', height: '100%', borderRadius: '50%', border: '4px solid rgba(255,255,255,0.05)', 
          display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 1,
          boxShadow: isRunning ? '0 0 40px rgba(139,92,246,0.4), inset 0 0 40px rgba(139,92,246,0.2)' : 'inset 0 0 20px rgba(0,0,0,0.5)',
          background: 'rgba(15,23,42,0.8)'
        }}>
          <Typography variant="h2" fontWeight={900} sx={{ background: 'linear-gradient(180deg, #FFFFFF, rgba(255,255,255,0.7))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', letterSpacing: '2px' }}>
      ...
      <Box sx={{ display: 'flex', gap: 3, alignItems: 'center' }}>
        <IconButton onClick={resetTimer} sx={{ color: 'rgba(255,255,255,0.4)', bgcolor: 'rgba(0,0,0,0.3)', width: 48, height: 48, '&:hover': { bgcolor: 'rgba(255,255,255,0.1)', color: 'white' } }}>
          <RotateCcw size={20} />
        </IconButton>
        <IconButton 
          onClick={toggleTimer} 
      ...
      <LinearProgress variant="determinate" value={progress} sx={{ width: '80%', mt: 4, height: 4, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.1)', '& .MuiLinearProgress-bar': { bgcolor: '#8B5CF6' } }} />
    </Box>
  );
}
