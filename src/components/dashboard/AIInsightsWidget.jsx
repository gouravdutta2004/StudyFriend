       ...
       <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, flex: 1 }}>
         {insights.map((item, i) => (
           <motion.div key={i} whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
              <Box sx={{ p: 3, borderRadius: '20px', bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 2, alignItems: 'flex-start', cursor: 'pointer', '&:hover': { borderColor: item.color, boxShadow: `0 10px 30px ${item.bg}` } }}>
                 <Box sx={{ bgcolor: item.bg, color: item.color, p: 1.5, borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                   {item.icon}
                 </Box>
                 <Box sx={{ flex: 1 }}>
                   <Typography variant="subtitle2" fontWeight={800} color="white" mb={0.5}>{item.title}</Typography>
                   <Typography variant="body2" color="rgba(255,255,255,0.6)" lineHeight={1.5}>{item.text}</Typography>
                 </Box>
                 <IconButton size="small" sx={{ color: 'rgba(255,255,255,0.3)' }}><ArrowUpRight size={16} /></IconButton>
              </Box>
           </motion.div>
         ))}
       </Box>
