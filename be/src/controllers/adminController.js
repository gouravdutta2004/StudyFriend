...
const broadcastEmail = async (req, res) => {
  try {
    ...
    if (filters) {
      if (filters.subject) query.subjects = { $in: [new RegExp(filters.subject, 'i')] };
...
const updateReport = async (req, res) => {
  try {
    const report = await Report.findByIdAndUpdate(req.params.id, { status: req.body.status, actionTaken: req.body.actionTaken }, { new: true });
    res.json(report);
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
const updateFlaggedItem = async (req, res) => {
  try {
    const item = await FlaggedItem.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true });
    res.json(item);
  } catch (err) { res.status(500).json({ message: err.message }); }
};
